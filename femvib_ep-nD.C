...
int main (int argc, char ** argv)
{
  ...
  // Becaue libMesh::ElemType is an enumeration type, I can't directly assign a string value to it.
  ...
  // 1D grid on a line.
  if (ndim == 1)
  MeshTools::Generation::build_line (mesh, nx, -x0, x0,
                                       elemType);       // EDGE2 EDGE3 EDGE4
  // 2D grid on a square.
  else if (ndim == 2)
  MeshTools::Generation::build_square (mesh, nx, ny, -x0, x0, -y0, y0,
                                       elemType);       // TRI3 TRI6 QUAD4 QUAD8 QUAD9
  // 3D grid on a cube.
  else if (ndim == 3)
  MeshTools::Generation::build_cube (mesh, nx, ny, nz, -x0, x0, -y0, y0, -z0, z0,
