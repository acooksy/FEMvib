#!/bin/bash
# 15-Oct-2024
# This script intended to convert an ExodusII text output file (converted to text using ncdump)
...
startline_v=$(($(awk '/vals_nod_.* =/ {print NR} ' "$file" | xargs)))
startline_x=$(awk '/coordx =/ {print NR} ' "$file")
startline_y=$(awk '/coordy =/ {print NR} ' "$file")
if [ "$ndim" -gt 2 ]; then
 startline_z=$(awk '/coordz =/ {print NR} ' "$file")
fi
...
str_blanklines=$(awk '/^$/ { ORS=" "; print NR} ' "$file")
...
# read in the lines
str_values=$(sed -n "$startline_v,$endline_v p" "$file" | xargs | tr -d '\n;')
IFS=',' read -ra values <<< "$str_values"
str_coordx=$(sed -n "$startline_x,$endline_x p" "$file" | xargs | tr -d '\n;')
IFS=', ' read -ra coordx <<< "$str_coordx"
str_coordy=$(sed -n "$startline_y,$endline_y p" "$file" | xargs | tr -d '\n;')
IFS=', ' read -ra coordy <<< "$str_coordy"
if [ "$ndim" -gt 2 ]; then
str_coordz=$(sed -n "$startline_z,$endline_z p" "$file" | xargs | tr -d '\n;')
