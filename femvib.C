...
// This is a BAD THING, using a global variable defined outside the functions,
// but I'm not able to edit the assemble_mass argument list to pass the number of dimensions that way
...
int main (int argc, char ** argv)
{
  ...
  ndim = 3;
  if (command_line.search(1, "-ndim"))
    ndim = command_line.next(ndim);
  ...
  float x1 = 1.0;
  if (command_line.search(1, "-x1"))
    x1 = command_line.next(x1);
  ...
  float y1 = 1.0;
  if (command_line.search(1, "-y1"))
    y1 = command_line.next(y1);
  ...
  float z1 = 1.0;
  if (command_line.search(1, "-z1"))
    z1 = command_line.next(z1);

  int pbc_x = 0;
  if (command_line.search(1, "-pbcx"))
    pbc_x  = command_line.next(pbc_x);

  int pbc_y = 0;
  if (command_line.search(1, "-pbcy"))
    pbc_y  = command_line.next(pbc_y);

  int pbc_z = 0;
  if (command_line.search(1, "-pbcz"))
    pbc_z  = command_line.next(pbc_z);

  std::string elemTypeStr = "HEX27";
  if (command_line.search(1, "-elemTypeStr"))
    elemTypeStr = command_line.next(elemTypeStr);
