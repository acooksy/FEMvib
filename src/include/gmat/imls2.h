...
void imls2(MatDoub &xmat, VecDoub &yvec, VecDoub &intPoint, Int &npt, Int &ndim, VecDoub &derivative)
{
    ...
    // 1D case scenario => our xmat has 1 columns
    if (ndim == 1)
    {
        Int rowb = 4;
        Int npts = 20;
        ...
        //  howmany(xmat, intPoint, ndim, npt, npts, cut, index);
        ...
        for (i=0; i<npts; i++)
        {
            distance = sqrt((intPoint[0] - pts[i][0])*(intPoint[0] - pts[i][0]));
            dc = distance; // dc is just a dummy variable to simplify equations
            sfunc = (1 - dc*dc*dc*dc*dc*dc);
            W[i][i] = sfunc * exp (-(distance*distance) ) / ( pow(distance,8)
                                                             + eps);
        }
        ...
        //  a0 + a1*x + a2*x^2 + a3*x^3
        derivative[0] = a[1] + 2*a[2]*intPoint[0] + 3*a[3]*intPoint[0]*intPoint[0];
      ...
      //  howmany(xmat, intPoint, ndim, npt, npts, cut, index);
        ...
        MatDoub B(npts,rowb);
            ...
            for (i=0; i<npts; i++) 
            {
                ...
                B[i][1] = pts[i][0];
                B[i][2] = pts[i][1];
                B[i][3] = pts[i][0]*pts[i][0];
                B[i][4] = pts[i][1]*pts[i][1];
                B[i][5] = pts[i][0]*pts[i][1];
                B[i][6] = pts[i][0]*pts[i][0]*pts[i][0];
                B[i][7] = pts[i][1]*pts[i][1]*pts[i][1];
            } // Matrix B assembled
    ...
    if (ndim == 3) 
    {
        ...
        for (i=0; i<npts; i++)
        {
            ...
            sfunc = (1 - dc*dc*dc*dc*dc*dc);
            W[i][i] = sfunc * exp (-(dc*dc) ) / ( pow(dc,8) + eps);
        }
