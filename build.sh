#!/bin/bash
# ALC 12-Aug-2025
# Edited to prepare femvib_run.sh with correct executable directory, 
#  expanded some messaging
#
# Edit the lines below to set the correct installation directories and other specifications
...
if [ ! -e ./tools/tune ] ; then
	flag=1
	echo "compiling tune" 
	g++ -I./include/tune/ -o tools/tune src/tune.cpp 
fi
if [ ! -e ./femvib ] ; then
	flag=1
	echo "compiling femvib.cpp" 
...
if [ ! -e ./femvib_run.sh ] ; then
	flag=1
	echo "installing execution script" 
	sed "s|DUMMY|${rootdir}/|" ./build/femvib_run.txt > ./femvib_run.sh
	chmod 755 ./femvib_run.sh
fi
if [ ${flag} -eq 0 ] ; then
	echo "nothing to do"
fi
